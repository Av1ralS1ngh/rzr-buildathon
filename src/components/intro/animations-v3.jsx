...
function Stage({
  ...
  // When false the stage neither reads nor writes the saved playhead — an
  // embedded run (app intro, kiosk) must start at 0 on every mount.
  persist = true,
  children,
}) {
  ...
  return (
    <React.Fragment>
      <Stage width={width} height={height} duration={derived.total} background={bg}
             autoplay={autoplay} loop={loop} playback={pb} persist={props.persist !== false}>
...
function useWatercolorLayers(painting, opts) {
  var kit = typeof window !== 'undefined' ? window.WatercolorKit : undefined;
